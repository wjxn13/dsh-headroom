...
// NOTE: @deepseek-ai/dsh-client-web-react was removed in dsh 0.1.5 (no longer a
// platform seed word nor a materialized module). Its only use — bindSnapshotSelector —
// was a thin useSyncExternalStore wrapper over SettingsScope.subscribe/getSnapshot,
// which still exist in 0.1.5. The hook is now inlined in apply() below.
...
function apply(ctx) {
  ...
  const useSnapshot = (selector) => {
    const cacheRef = import_react.useRef({ src: void 0, out: void 0 });
    const selRef = import_react.useRef(selector);
    selRef.current = selector;
    const subscribe = import_react.useCallback((onChange) => scope.subscribe(onChange), [scope]);
    ...
    return import_react.useSyncExternalStore(subscribe, getSnapshot, getSnapshot);
  };
